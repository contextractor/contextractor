...
`request_queues/` dirs. Same as running `extract --purge` before a crawl.
...
| `--max-crawl-depth`          | Max link depth from start URLs (0 = unlimited)                                                                                                                                                                                                         |
...
| `--save`                     | Format-destination token, e.g. markdown-kvs, original-dataset (repeatable). Format: txt\|markdown\|json\|html\|original; destination: dataset\|kvs. List a format twice to save to both. Saving original/html to the dataset risks OOM on large pages. |
...
- `runPurgeAction(opts?: PurgeOpts): Promise<PurgeResult>` — clears the
  ...
  subcommand's behavior). `opts`: `storageDir`. Returns `{ storageDir }` with
  the resolved path.
